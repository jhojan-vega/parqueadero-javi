...
export function DashboardPage({ activeSection, role, onNavigate, token, collaboratorId, onServiceAction }: DashboardPageProps) {
  ...
  return <div className="dashboard-page">
    ...
    <section className="dashboard-detail-grid">
      <article className="movements-card"><div className="card-title"><div><span className="eyebrow">SERVICIOS</span><h2>Servicios activos</h2></div><button type="button" onClick={() => onNavigate('Servicios')}>Ver servicios</button></div><div className="table-wrap"><table><thead><tr><th>Recibo</th><th>Tipo</th><th>Placa / ID</th><th>Entrada</th><th>Estado</th></tr></thead><tbody>{services.slice(0, 5).map((service) => <tr key={service.id_servicio}><td><strong>{service.codigo_recibo}</strong></td><td><VehicleTypeIndicator type={service.tipo_vehiculo} compact /></td><td>{service.placa ? <Plate value={service.placa} /> : <span className="identifier">{service.identificacion_usuario}</span>}</td><td>{new Intl.DateTimeFormat('es-CO', { hour: '2-digit', minute: '2-digit' }).format(new Date(service.fecha_hora_entrada))}</td><td><span className="status active-status">Activo</span></td></tr>)}{services.length === 0 && <tr><td colSpan={5}>No hay servicios activos.</td></tr>}</tbody></table></div></article>
