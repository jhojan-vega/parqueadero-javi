...
export function DashboardPage({ activeSection, role, onNavigate, token, collaboratorId, onServiceAction }: DashboardPageProps) {
  ...
  const availabilityItems = availability ? [{ label: 'Carros', icon: '▰', occupied: availability.Carro.ocupados, capacity: availability.Carro.capacidad }, { label: 'Motos', icon: '◉', occupied: availability.Moto.ocupados, capacity: availability.Moto.capacidad }, { label: 'Bicicletas', icon: '⌁', occupied: availability.Bicicleta.ocupados, capacity: availability.Bicicleta.capacidad }] : []
  ...
  return <div className="dashboard-page">
    ...
    <section className="quick-actions" aria-label="Acciones principales">
      <button type="button" onClick={() => onServiceAction('entry')}><span>→</span><b>Registrar ingreso</b><small>Nuevo servicio</small></button>
      <button type="button" onClick={() => onServiceAction('exit')}><span>←</span><b>Registrar salida</b><small>Consultar y cobrar</small></button>
      <button className="secondary-action" type="button" onClick={() => onNavigate(role === 'Vigilante' ? 'Caja' : 'Cajas')}><span>▤</span><b>{role === 'Vigilante' ? 'Mi Caja' : 'Ver cajas'}</b><small>Gestión de turnos</small></button>
    </section>
