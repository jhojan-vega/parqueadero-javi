...
import { cambiarContrasena, confirmarRecuperacion, iniciarSesion, restablecerContrasena, solicitarRecuperacion } from './auth.service';
...
export const rutasAuth = async (app: FastifyInstance): Promise<void> => {
  ...
  app.patch('/auth/cambiar-contrasena', { preHandler: [autenticar] }, async (solicitud, respuesta) => {
    const b=solicitud.body; if(!esObjeto(b)||Object.keys(b).length!==3||typeof b.contrasena_actual!=='string'||typeof b.nueva_contrasena!=='string'||typeof b.confirmacion_nueva_contrasena!=='string'||b.nueva_contrasena!==b.confirmacion_nueva_contrasena) return respuesta.status(400).send({mensaje:'Datos de contraseña inválidos'});
  ...
  app.post('/auth/recuperacion/solicitar', async (solicitud, respuesta) => {
    ...
    try { await enviarCodigoRecuperacion(b.correo.trim(),r.codigo!); return respuesta.send({mensaje:'Si el correo es elegible, se enviará un código'}); } catch(e) { if(e instanceof CorreoNoConfiguradoError) return respuesta.status(503).send({mensaje:'Servicio de correo no configurado'}); throw e; }
  });

  app.post('/auth/recuperacion/confirmar', async (solicitud, respuesta) => {
    const b=solicitud.body; if(!esObjeto(b)||Object.keys(b).length!==4||typeof b.correo!=='string'||typeof b.codigo!=='string'||!/^\d{6}$/.test(b.codigo)||typeof b.nueva_contrasena!=='string'||typeof b.confirmacion_nueva_contrasena!=='string'||b.nueva_contrasena!==b.confirmacion_nueva_contrasena) return respuesta.status(400).send({mensaje:'Datos de recuperación inválidos'});
  ...
  app.post<{ Params: { id: string } }>('/auth/restablecer/:id', { preHandler: [autenticar, autorizarRoles(...rolesAdministrativos)] }, async (solicitud, respuesta) => {
    const b=solicitud.body; if(!/^\d+$/.test(solicitud.params.id)||!esObjeto(b)||Object.keys(b).length!==2||typeof b.contrasena_temporal!=='string'||typeof b.confirmacion_contrasena_temporal!=='string'||b.contrasena_temporal!==b.confirmacion_contrasena_temporal) return respuesta.status(400).send({mensaje:'Datos de restablecimiento inválidos'});
